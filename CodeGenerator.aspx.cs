...
public partial class CodeGenerator : System.Web.UI.Page
{
    ...
    //    return null; // Or handle the case where no SQL Server instances are found
    //}
    ...
    protected void GrdLoadColumnNames(string selectedTableName)
    {
    ...
    protected void PopulateDropDownListsInGridView()
    {
    ...
    protected void ddlListControlType(DropDownList ddl)
    {
        ...
        using (SqlConnection connection = new SqlConnection(connectionString))
        {
            using (SqlDataAdapter adapter = new SqlDataAdapter(query, connection))
            {
                ...
                // Filter out NULL values from the DataTable
                DataTable filteredDt = dt.AsEnumerable()
                    .Where(rows => !rows.IsNull("ControlType"))
                    .CopyToDataTable();
                ddl.DataSource = filteredDt;
    ...
    {
        string selectedTableName = ddlTables.SelectedValue;
        GrdLoadColumnNames(selectedTableName);
        PopulateDropDownListsInGridView();
    }
    protected void btnGenerateAspx_Click(object sender, EventArgs e)
    {
        ...
        if (isCheckViewChecked)
        {
            ...
            // Add the columns dynamically to the dynamicPageContent string
            dynamicPageContent += "columns: [\n";
        ...
        if (isCheckViewChecked)
        {
            ...
            foreach (GridViewRow row in GridViewColumns.Rows)
            {
                ...
                CheckBox chkEdit = row.FindControl("CheckView") as CheckBox;
                if (chkEdit != null && chkEdit.Checked)
                {
        ...
        if (isCheckEditChecked)
        {
            ...
            foreach (GridViewRow row in GridViewColumns.Rows)
            {
                ...
                if (chkBox != null && chkBox.Checked )
                {
                    ...
                    {
                        case "FileUpload":
                            codeBehindContent += "string fuEdit" + columnName + "= \"\";\n";
                            break;
                    }
                }
            }
